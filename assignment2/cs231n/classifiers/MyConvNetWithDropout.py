...
class MySixLayerConvNet2(object):
  """
  ...
  def __init__(self, input_dim=(3, 32, 32), num_filters=(32, 64, 32, 64), filter_size=3,
               hidden_dim=100, num_classes=10, weight_scale=1e-3, reg=0.0, use_batchnorm=False, dropout=0, seed=None,
               dtype=np.float32):
    """
  ...
  def loss(self, X, y=None):
    ...
    [out1_d, d_cache1] = dropout_forward(out1, self.dropout_param)
    [out2, cache_layer2] = conv_batchnorm_relu_pool_forward(out1_d, W2, b2, conv_param, pool_param, self.params['gamma2'], self.params['beta2'], self.bn_params[1])
    [out2_d, d_cache2] = dropout_forward(out2, self.dropout_param)
    [out3, cache_layer3] = conv_batchnorm_relu_pool_forward(out2_d, W3, b3, conv_param, pool_param, self.params['gamma3'], self.params['beta3'], self.bn_params[2])
    [out3_d, d_cache3] = dropout_forward(out3, self.dropout_param)
    [out4, cache_layer4] = conv_batchnorm_relu_pool_forward(out3_d, W4, b4, conv_param, pool_param, self.params['gamma4'], self.params['beta4'], self.bn_params[3])
    [out4_d, d_cache4] = dropout_forward(out4, self.dropout_param)
    [out5, cache_layer5] = affine_batchnorm_relu_forward(out4_d, W5, b5, self.params['gamma5'], self.params['beta5'], self.bn_params[4])
    out5_d, d_cache5 = dropout_forward(out5, self.dropout_param)
    [scores, cache_layer6] = affine_forward(out5_d,W6,b6)
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    dout5_d, grads['W6'], grads['b6'] = affine_backward(dscores,cache_layer6)
    grads['W6'] += self.reg*W6
    dout5 = dropout_backward(dout5_d, d_cache5)
    dout4_d, grads['W5'], grads['b5'], grads['gamma5'], grads['beta5'] = affine_batchnorm_relu_backward(dout5, cache_layer5)
    grads['W5'] += self.reg*W5
    dout4 = dropout_backward(dout4_d, d_cache4)
    dout3_d, grads['W4'], grads['b4'], grads['gamma4'], grads['beta4'] = conv_batchnorm_relu_pool_backward(dout4, cache_layer4)
    grads['W4'] += self.reg*W4
    dout3 = dropout_backward(dout3_d, d_cache3)
    dout2_d, grads['W3'], grads['b3'], grads['gamma3'], grads['beta3'] = conv_batchnorm_relu_pool_backward(dout3, cache_layer3)
    grads['W3'] += self.reg*W3
    dout2 = dropout_backward(dout2_d, d_cache2)
    dout1_d, grads['W2'], grads['b2'], grads['gamma2'], grads['beta2'] = conv_batchnorm_relu_pool_backward(dout2, cache_layer2)
    grads['W2'] += self.reg*W2
    dout1 = dropout_backward(dout1_d, d_cache1)
